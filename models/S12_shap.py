...
# -----------------------------
# PLOT FIXED: PERCENTAGES ALWAYS ABOVE BAR TIP (POS/NEG COLORS)
# -----------------------------
...
# Update traces: always show text above bar tip
fig.update_traces(
    texttemplate="<b style='color:black;'>%{text:.1f}%</b>",
    textposition='outside',  # force above bar tip for all bars
    textfont_size=12,
    cliponaxis=False
)

# Extend y-axis so negative bars have room above the tip
y_min = plot_df['SHAP Impact Rate (%)'].min() * 1.2 if plot_df['SHAP Impact Rate (%)'].min() < 0 else 0
