...
# --------------------------------------------------
# PLOT WITH PERCENTAGES ABOVE BAR TIPS (POS/NEG COLORS)
# --------------------------------------------------
import plotly.express as px
...
# Calculate adjusted text positions
# We'll use 'outside' for positive bars
# and slightly above the negative bar tip for negative bars
text_positions = ['outside' if val >= 0 else 'inside' for val in plot_df['SHAP Impact Rate (%)']]
...
fig = px.bar(
    ...
    category_orders={"Feature": plot_df["Feature"].tolist()}  # maintain feature order
)
...
# Layout with black labels and legend
fig.update_layout(
