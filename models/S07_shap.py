...
# -----------------------------
st.divider()
st.header('📊 SHAP Feature Impact by Family (Vertical Bars, % of total)')
...
# -----------------------------
families = {}
# Numerical features: each its own family
...
family_df = pd.DataFrame({
    'Family': list(family_shap.keys()),
    'SHAP Value (%)': list(family_shap.values())
})
family_df['Color'] = family_df['SHAP Value (%)'].apply(lambda x: 'green' if x>=0 else 'red')
family_df = family_df.sort_values('SHAP Value (%)')
...
fig.update_layout(
    yaxis_title="<b>SHAP Impact (% of total)</b>",
    xaxis_title="<b>Feature Family</b>",
    legend_title="<b>Impact Direction</b>"
)
