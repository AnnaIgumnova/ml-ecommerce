...
st.divider()
st.header("📊 Feature Impact & Influence on Conversion Rate for This Session")
...
df["Color"] = df["Impact"].apply(lambda x: "green" if x >= 0 else "red")
...
fig.update_layout(
    yaxis_title="Impact on Conversion Probability",
    xaxis_title="Feature",
    showlegend=False
)

max_val = df["Impact"].abs().max()
gap = max_val * 0.08
...
st.info(
    ...
    f"({top_feature['Influence']:.1f}% influence, impact {top_feature['Impact']:.2f})"
)