...
# --------------------------------------------------
# FEATURE MAX VALUES (from dataset knowledge)
# --------------------------------------------------
...
# --------------------------------------------------
# SHAP EXPLAINER (works without dataset)
# --------------------------------------------------

background = pd.DataFrame(
    np.zeros((1, len(input_df.columns))),
...
if data["prod_related"] > 40:
    st.success("Customer shows strong product interest.")

elif data["prod_related"] > 15:
    st.info("Customer is comparing products.")

else:
    st.warning("Limited product interaction.")

if data["bounce_rate"] > 0.10:
    st.warning("High bounce rate detected.")

if data["exit_rate"] > 0.12:
    st.warning("Customer may exit soon.")
...
if month in ["Nov","Dec"]:
    st.success("Peak shopping season.")
...
if purchase_prob > 70:
    st.success("Promote complementary products.")

elif purchase_prob > 50:
    st.info("Offer limited-time discount.")

elif purchase_prob > 30:
    st.write("Retarget visitor with ads.")

else:
    st.warning("Focus on engagement campaigns.")
...
top_features = contrib_df.sort_values("abs", ascending=False).head(10)

st.subheader("Top Drivers of Conversion")
...
positive = contrib_df.sort_values("Contribution", ascending=False).head(3)
negative = contrib_df.sort_values("Contribution").head(3)

st.write("### Features Increasing Conversion")

for _,row in positive.iterrows():
    st.success(f"{row['Feature']} increases purchase likelihood")

st.write("### Features Reducing Conversion")
