...
# Identify current session categorical features
...
family_df = family_df.sort_values('SHAP Impact Rate (%)')

# --------------------------------------------------
# PLOT WITH PERCENTAGES ALWAYS ABOVE BAR TIP (POS/NEG COLORS)
# --------------------------------------------------

# Copy the SHAP family dataframe
...
fig = px.bar(
    ...
    category_orders={"Feature": plot_df["Feature"].tolist()}  # keep original order
)

# Force all text outside the bar, always on top
fig.update_traces(
...
# Expand y-axis so negative bars' percentages appear above x-axis
y_min = min(0, plot_df["SHAP Impact Rate (%)"].min() * 1.2)
y_max = plot_df["SHAP Impact Rate (%)"].max() * 1.2

fig.update_layout(
    xaxis_title="<b style='color:black;'>Feature</b>",
    yaxis=dict(
        title="<b style='color:black;'>Impact Rate (%)</b>",
...
Feature Contribution to Session Conversion
...
traffic_type            ████ 6%
exit_rate               ███ 4%
prod_related_duration   ██ 3%



| 📅 Month | 👤 Visitor  | 🚦 Traffic     | 🌍 Region | 🧭 Browser | 💻 OS | 🎉 Special Day | 📅 Weekend | 🛍 Products Viewed | ⏱ Browsing Time | 📉 Bounce Rate | 💰 Page Value |
| -------- | ----------- | -------------- | --------- | ---------- | ----- | -------------- | ---------- | ------------------ | --------------- | -------------- | ------------- |
| Aug      | New_Visitor | traffic_type_1 | region_1  | browser_1  | os_1  | No             | No         | 30                 | 16 min 26 sec   | 3.0%           | 36            |


| 👤 Visitor	 | 🚦 Traffic Type	| 🌍 Region	| 🧭 Browser	| 💻 OS	| 📅 Weekend  |  📅 Month || 🛍 Products Viewed | ⏱ Browsing Time | 📉 Bounce Rate | Exit Rate  |💰 Page Value |
| -------- | ----------- | -------------- | --------- | ---------- | ----- | --------------  | ------------------ | --------------- | -------------- | ------------- |------------- |
